...
use arc_web::ui::{AdminNavItem, Audience, Breadcrumb, TemplateBundle, TemplateDef, TemplateName, UiContribution, UiHost, UiPage};
...
fn render(registry:&UiRegistry,req:&HttpRequest,session:&Session,name:&'static str,title:&str,mut context:Context,breadcrumbs:Vec<Breadcrumb>)->HttpResponse{context.insert("title",title);registry.render(UiPage{template:TemplateName(name),title:title.into(),context,status:actix_web::http::StatusCode::OK,breadcrumbs},req,session)}

#[get("/")]
async fn home(req:HttpRequest,session:Session,registry:web::Data<UiRegistry>)->impl Responder { render(&registry,&req,&session,"home.html","Home",Context::new(),vec![]) }
async fn dashboard(req:HttpRequest,session:Session,registry:web::Data<UiRegistry>)->impl Responder { let mut context=Context::new();context.insert("stats",&HashMap::from([("events",0),("projections",0)]));render(&registry,&req,&session,"admin/dashboard.html","Workbench",context,vec![]) }
