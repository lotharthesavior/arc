...
2. **HIPAA-5 — Event signature persistence/enforcement.** Integrity primitives exist, but SQLite events do not persist signatures or verify the integrity chain on append/load.
3. **HIPAA-2b — Mechanical read-logging guarantee.** Runtime access logging exists, but new regulated read controllers can still return PHI/PII without calling `record_read`.
...
- [~] **HIPAA-5** Integrity primitives complete; store enforcement deferred. `IntegrityChain` trait + `HmacSha256Chain` reference impl + canonical event byte format live in `arc-core::integrity`. `EventSignature` and `verify_chain` are tested, but SQLite events do not yet persist signatures or verify the chain on append/load. §164.312(c)(1). Docs: `docs/guides/integrity-chain.md` + `architecture-23-integrity-chain` diagram.
...
1. **Step 5 follow-through** — run `ARC_POSTGRES_TEST_DATABASE_URL=postgres://... cargo test -p arc-es-postgres`, exercise `DATABASE_DRIVER=postgres` app/worker startup, and document Postgres schema initialization / migration boundaries.
