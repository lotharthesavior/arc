...
"${compose[@]}" exec -T verify cargo test --locked -p arc-core -p arc-web -p arc-auth-rbac
...
"${compose[@]}" exec -T verify cargo clippy --locked -p arc-core -p arc-web -p arc-auth-rbac -p arc --all-targets --all-features -- -D warnings
