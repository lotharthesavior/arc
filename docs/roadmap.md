...
- [x] **Extract web layer to arc-web** ✅
  - Framework runtime (Actix bootstrap, middleware, helpers, CLI commands, base routes, websocket, shipped auth/admin/API/diag controllers, internal projection HTTP) lives in `crates/arc-web`.
  - Thin `crates/arc-app` (package `arc`) depends on `arc-web` via `ArcApp::builder().register_routes(...).run()`.
  - Ownership enforced by ADR 0002 + `make doctor` structural boundary check.
  - **Status**: Complete (in-tree split). Publishing `arc-web` and `arc new` remain Framework Readiness follow-ups.
...
- [x] **Step 4: Benthos durable routing layer** ✅
  - Benthos consumes `events.>` from NATS JetStream and owns routing, filtering, dedupe, retry, dead-lettering, and event-handler delivery. Benthos must never write directly to Arc databases; projection writes stay inside Arc-owned handlers/services.
  - **Files**: `config/benthos/events.yaml`, `config/benthos/generated/events.yaml`, `scripts/generate-benthos-config.mjs`
  - **Status**: Base pipeline, generated runtime pipeline, Compose service, handler-manifest generator, `benthos lint` CI gate, envelope validation, enriched `x_arc_dlq` metadata, SQL/database delivery rejection, publish → Benthos → Arc HTTP projection handler/service → read-model integration coverage, DLQ/redrive operator docs, and automatic `ARC_DLQ` provisioning for `dlq.>` are complete.
...
- Goal: Use Benthos for event routing, filtering, handler delivery, retries, and DLQ handling, while keeping Rust focused on domain logic and storage.
- `arc-es-nats` remains the publishing side.
- Framework users extend event handling through handler manifests and external services; they do not edit a Rust consumer crate. Manifests support HTTP/NATS delivery, not SQL/database delivery.
...
### 1.5 Production Storage Driver Switch (Complete)
...
- [x] **Step 5: Postgres event/read-model stores** ✅
  ...
  - **Status**: Complete (validated against live DB, app store wiring verified). Durable routing runs in Benthos, while projection writes remain in Arc-owned code paths (ADR 0001).
...
**Updated**: 2026-07-08 (arc-web split + upgrade contract reconciliation)
...
6. ✅ Build out Benthos handler-manifest generation, config artifacts, linting, envelope validation, enriched DLQ metadata, and SQL/database delivery rejection.
7. ✅ Add publish → Benthos → Arc HTTP projection handler/service → read-model integration coverage.
8. ✅ Document DLQ/redrive operator workflow.
9. ✅ Provision `ARC_DLQ` (`dlq.>`) automatically so Benthos DLQ messages persist in NATS JetStream.
10. 🔲 Continue documentation/reference reconciliation.
...
3. ✅ Align first-pass handler manifest contract with generated config: reject unsupported ordering/key/backoff values, reject unknown keys, retry supported delivery targets, and reject SQL/database delivery.
4. ✅ Add a publish → Benthos → Arc HTTP projection handler/service → read-model integration test.
...
8. 🔲 Run adapter crate publish dry-runs after `arc-core` is published or available in the registry cache.
...
0. ✅ Upgrade contract defined and accepted: `docs/adr/0002-framework-upgrade-contract.md` covers the machine-owned/Arc-owned/user-owned file boundary, the lockstep SemVer + public API policy (traits, event envelope, handler manifests, subject naming, shipped migrations), and the supported upgrade path. Ownership tables updated for the landed `arc-web` / thin `arc-app` split.
1. ✅ `make doctor` / `make arc-check` drift guard: generated Benthos config ownership banner + freshness, **and** structural Arc-owned boundary (framework runtime trees must live in `crates/arc-web`, not under `crates/arc-app/src`).
2. ✅ Split reusable framework web/runtime into `crates/arc-web`; `crates/arc-app` is the thin user template (`ArcApp::builder()`).
3. 🔲 Publish the versioned `arc-*` crates under the ADR 0002 SemVer/public API policy — including **`arc-web`** (in-tree split does not publish crates).
4. ✅ `docs/guides/upgrading.md` — user-facing upgrade path (bump versions, migrate, regenerate Benthos, `make doctor`, test/e2e); normative contract remains ADR 0002.
5. 🔲 Ship **`arc new` CLI** (or equivalent) so apps start from a thin template depending on versioned crates instead of clone-and-edit.
6. 🔲 Reposition top-level docs from web starter to upgradeable event-sourced framework.
...
| 2026-06-19 | 1.9 | Hard architecture direction recorded: Benthos must never write directly to Arc databases. Handler manifests now support HTTP/NATS delivery only and reject SQL/database delivery; distributed projection integration should prove Benthos calls an Arc-owned HTTP projection handler/service, which then updates read models through Arc storage code | Status reconciliation |
| 2026-06-21 | 2.0 | Benthos routing maturity follow-ups completed: publish→Benthos→Arc HTTP projection handler→read-model integration coverage added, DLQ/redrive operator workflow documented, and `arc-es-nats` provisions `ARC_DLQ` for durable `dlq.>` persistence | Status reconciliation |
| 2026-06-21 | 2.1 | Publishable crate metadata prepared for `arc-core`, `arc-es-sqlite`, `arc-es-postgres`, and `arc-es-nats`; `arc-core` passed publish dry-run. Adapter dry-runs are gated on publishing or caching `arc-core` first | Status reconciliation |
| 2026-07-01 | 2.2 | Framework upgrade contract accepted as ADR 0002: machine-owned/Arc-owned/user-owned file boundary, lockstep SemVer + public API surface, and supported upgrade path. Framework Readiness track reconciled against it | Framework readiness |
| 2026-07-08 | 2.3 | `arc-web` / thin `arc-app` split landed; ADR 0002 ownership tables and Known boundary debt updated; `make doctor` enforces Arc-owned structural boundary; `docs/guides/upgrading.md` added. Remaining Framework Readiness: publish `arc-web`, ship `arc new` | Framework readiness |
